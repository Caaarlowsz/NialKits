...
/*     */ {
	/*     */ public static String formatColors(String str) {
		/* 8 */ char[] chars = { '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', 'a', 'b', 'c', 'd', 'e', 'f', 'n',
				'r', 'l', 'k', 'o', 'm' };
		/* 9 */ char[] array = str.toCharArray();
		/* 10 */ for (int t = 0; t < array.length - 1; t++) {
			/* 11 */ if (array[t] == '&') {
				char[] arrayOfChar1;
				/* 12 */ int j = (arrayOfChar1 = chars).length;
				for (int i = 0; i < j; i++) {
	...
	/*     */ public static void box(String[] paragraph, String title) {
		...
		/* 40 */ at = "";
		/* 41 */ buffer.add("|                                                   |");
		/* 42 */ String[] arrayOfString = paragraph;
		...
		/* 67 */ j = (arrayOfString = (String[]) buffer.toArray(new String[buffer.size()])).length;
		for (int i = 0; i < j; i++) {
	...
	/*     */ public static boolean compareVersion(String old, String newer) {
		...
		/* 100 */ String at = "";
		/* 101 */ char[] arrayOfChar;
		int j = (arrayOfChar = old.toCharArray()).length;
		for (int i = 0; i < j; i++) {
		...
		/*     */ try {
			...
			/*     */ }
		/* 118 */ at = "";
		/* 119 */ j = (arrayOfChar = newer.toCharArray()).length;
		for (int i = 0; i < j; i++) {
			char c = arrayOfChar[i];
			/* 120 */ if (c != '.') {
				/* 121 */ at = at + c;
