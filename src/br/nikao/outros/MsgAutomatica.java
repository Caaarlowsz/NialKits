	...
	/*    */ {
		/* 14 */ Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(Main.getInstance(), new Runnable() {
			/*    */ public void run() {
				...
				/* 16 */ int j = (arrayOfPlayer = Bukkit.getOnlinePlayers()).length;
				for (int i = 0; i < j; i++) {
				...
				/* 22 */ Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(Main.getInstance(), new Runnable() {
					/*    */ public void run() {
						Player[] arrayOfPlayer;
						/* 24 */ int j = (arrayOfPlayer = Bukkit.getOnlinePlayers()).length;
						for (int i = 0; i < j; i++) {
							Player s = arrayOfPlayer[i];
							/* 25 */ s.sendMessage("");
							/* 26 */ s.sendMessage(
									Main.getInstance().getConfig().getString("Mensagem2").replace("&", "§"));
							/* 27 */ s.sendMessage("");
							/*    */ }
						/*    */ }
					/* 30 */ }, 1200L);
				...
				/* 32 */ Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(Main.getInstance(), new Runnable() {
					/*    */ public void run() {
						Player[] arrayOfPlayer;
						/* 34 */ int j = (arrayOfPlayer = Bukkit.getOnlinePlayers()).length;
						for (int i = 0; i < j; i++) {
							Player s = arrayOfPlayer[i];
							/* 35 */ s.sendMessage("");
							/* 36 */ s.sendMessage(
									Main.getInstance().getConfig().getString("Mensagem3").replace("&", "§"));
							/* 37 */ s.sendMessage("");
							/*    */ }
						/*    */ }
					/* 40 */ }, 2400L);
				...
				/* 42 */ Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(Main.getInstance(), new Runnable() {
					/*    */ public void run() {
						...
						for (int i = 0; i < j; i++) {
							...
							/* 45 */ s.sendMessage("");
							/* 46 */ s.sendMessage(
									Main.getInstance().getConfig().getString("Mensagem4").replace("&", "§"));
							/* 47 */ s.sendMessage("");
							/*    */ }
						/*    */ }
					/* 50 */ }, 4600L);
				...
				/* 52 */ Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(Main.getInstance(), new Runnable() {
					/*    */ public void run() {
						Player[] arrayOfPlayer;
						/* 54 */ int j = (arrayOfPlayer = Bukkit.getOnlinePlayers()).length;
						for (int i = 0; i < j; i++) {
							Player s = arrayOfPlayer[i];
							/* 55 */ s.sendMessage("");
							/* 56 */ s.sendMessage(
									Main.getInstance().getConfig().getString("Mensagem5").replace("&", "§"));
							/* 57 */ s.sendMessage("");
							/*    */ }
						/*    */ }
					/* 60 */ }, 5800L);
...
/*
 * Location:
 * C:\Users\Vini\Downloads\KitPvP.jar!\br\nikao\outros\MsgAutomatica.class Java
