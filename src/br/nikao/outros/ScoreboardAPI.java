...
/*    */ {
	...
	/*    */ {
		...
		/* 52 */ if (Mensagem.length() <= 16) {
			/* 53 */ return new AbstractMap.SimpleEntry<Team, String>(null, Mensagem);
			/*    */ }
		/* 55 */ Team Time = this.Scoreboard.registerNewTeam("text-" + this.Scoreboard.getTeams().size());
		/* 56 */ Iterator<String> MensagemSplit = Splitter.fixedLength(16).split(Mensagem).iterator();
		...
		/* 63 */ return new AbstractMap.SimpleEntry<Team, String>(Time, Resultado);
		/*    */ }
	...
	/*    */ {
		...
		/* 72 */ for (Map.Entry<String, Integer> Mensagem : this.Scores.entrySet()) {
			/* 73 */ Map.Entry<Team, String> Time = criarTimes((String) Mensagem.getKey());
			/* 74 */ Integer Score = Integer
					.valueOf(Mensagem.getValue() != null ? ((Integer) Mensagem.getValue()).intValue() : index);
			/* 75 */ String Jogador = (String) Time.getValue();
			/*    */
			/* 77 */ if (Time.getKey() != null)
...
/*
 * Location:
 * C:\Users\Vini\Downloads\KitPvP.jar!\br\nikao\outros\ScoreboardAPI.class Java
