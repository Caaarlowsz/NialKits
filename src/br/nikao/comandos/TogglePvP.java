	...
	/*    */ {
		...
		/* 17 */ if ((cmd.getName().equalsIgnoreCase("togglepvp")) && ((p.hasPermission("ng.pvp")) || (p.isOp()))) {
			/* 18 */ if (p.getWorld().getPVP()) {
				...
				/* 20 */ org.bukkit.Bukkit.getServer()
						.broadcastMessage("§4§lATENÇÃO: §7§lAgora o pvp do servidor está: §cDesativado");
				/* 21 */ return true;
				/*    */ }
			/* 23 */ p.getWorld().setPVP(true);
			/* 24 */ org.bukkit.Bukkit.getServer()
					.broadcastMessage("§4§lATENÇÃO: §7§lAgora o pvp do servidor está: §aAtivado");
			/* 25 */ return true;
			/*    */ }
		/* 27 */ return true;
		/*    */ }
