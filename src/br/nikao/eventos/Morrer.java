...
/*    */ {
	...
	/*    */ {
		...
		/* 24 */ if ((e.getEntity().getKiller() instanceof Player)) {
			...
			/* 29 */ Main.Deaths.put(p.getName(),
					Integer.valueOf(((Integer) Main.Deaths.get(p.getName())).intValue() + 1));
			...
			/* 41 */ Main.Kills.put(t.getName(),
					Integer.valueOf(((Integer) Main.Kills.get(t.getName())).intValue() + 1));
		...
		/*    */ {
			...
			/* 54 */ if (VoidChallenge.voidchallenge.contains(p.getName())) {
				...
				/* 56 */ if (p.getLevel() > record) {
					/* 57 */ Main.getInstance().getConfig().set("Status." + p.getUniqueId() + ".VC",
							Integer.valueOf(p.getLevel()));
					/* 58 */ p.sendMessage(
							"§7§lVocê fez um novo record no VoidChallenge: §e" + p.getLevel() + " segundos.");
					/*    */ }
				/* 60 */ p.setExp(0.0F);
				/* 61 */ p.setLevel(0);
				...
				/* 71 */ Main.Deaths.put(p.getName(),
						Integer.valueOf(((Integer) Main.Deaths.get(p.getName())).intValue() + 1));
				/* 72 */ Main.XP.put(p.getName(),
						Integer.valueOf(((Integer) Main.XP.get(p.getName())).intValue() - 30));
